...
def run(args):
    ...
    print(f"  - Crop Size: {args.crop_size}x{args.crop_size} (latent: {args.crop_size//8}x{args.crop_size//8})")
    print(f"{'='*60}\n")
    ...
    # solve problem
    crop_size = args.crop_size  # 64x64 crop for full-NFE backprop
    tf = transforms.Compose([
        transforms.Resize(args.img_size),       # resize longer edge to img_size first
        transforms.CenterCrop(args.img_size),   # square crop at img_size
        transforms.Resize(crop_size),           # then downsample to crop_size (64x64)
        transforms.ToTensor()
    ])
    ...
    for i, path in enumerate(pbar):
        ...
        if args.task == 'deblur_motion':
            ...
            A_funcs = Deblurring_fft(kernel / kernel.sum(), 3, crop_size, solver.transformer.device)
        ...
        lH = crop_size // solver.vae_scale_factor
        lW = crop_size // solver.vae_scale_factor
        lC = solver.transformer.config.in_channels
        noise_shape = (1, lC, lH, lW)  # e.g. (1, 16, 8, 8) for 64x64
        ...
        if args.task in ['sr_bicubic', 'inpainting', 'inpainting_DIV2K', 'cs_walshhadamard', 'cs_blockbased']: # modify
            y_vis = A_funcs.At(y).reshape(1, 3, crop_size, crop_size)
        else:
            y_vis = y.reshape(1, 3, crop_size, crop_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--crop_size', type=int, default=64,
                        help='Crop input image to this size before experiment (e.g. 64). '
                             'Latent will be crop_size//8.')
    # workdir params
    ...
    parser.add_argument('--inner_NFE', type=int, default=28,
                        help='NFE for Euler sampling inside noise opt loop (default=28 = full chain for 64x64)')

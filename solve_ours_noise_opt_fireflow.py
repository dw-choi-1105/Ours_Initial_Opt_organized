...
def run(args):
    ...
    print(f"  - Optimization Sampler: Fireflow (not Euler)")
    print(f"{'='*60}\n")
    ...
    for i, path in enumerate(pbar):
        ...
        for opt_iter in range(args.noise_opt_steps):
            ...
            if args.use_amp:
                ...
                with autocast(dtype=torch.float16):
                    z0t_hat = solver.fireflow_sample(
        ...
        # ══════════════════════════════════════════════════════════════
        # 4-1. Final FireFlow Sampling
        # ══════════════════════════════════════════════════════════════
        print(f"\n  Final FireFlow sampling (NFE={args.NFE}) ...")
        solver.scheduler.config.shift = 4.0
        solver.scheduler.set_timesteps(args.NFE + 1, device=solver.transformer.device)
        final_timesteps = solver.scheduler.timesteps
        final_sigmas = final_timesteps.float() / solver.scheduler.config.num_train_timesteps
        
        # fireflow_sample_w_process returns (final_img, process_images)
        recon_img_fireflow, fireflow_process_images = solver.fireflow_sample_w_process(
        ...
        recon_img_fireflow = solver.decode(recon_img_fireflow).float()
        recon_img_fireflow_01 = (recon_img_fireflow / 2 + 0.5).clamp(0, 1)
        
        # ═══════════════════════════════════════════════════════════════
        # Save FireFlow sampling process grid
        # ═══════════════════════════════════════════════════════════════
        args.workdir.joinpath('progress_fireflow').mkdir(parents=True, exist_ok=True)
        if len(fireflow_process_images) > 0:
            # Concatenate all process images
            fireflow_progress_tensors = torch.cat(fireflow_process_images, dim=0)
            n_total = fireflow_progress_tensors.shape[0]
            nrow = min(n_total, max(1, int(math.ceil(math.sqrt(n_total)))))
            fireflow_progress_grid = make_grid(fireflow_progress_tensors, nrow=nrow, padding=2, pad_value=1.0)
            save_image(fireflow_progress_grid, args.workdir / 'progress_fireflow' / f'{img_name}_fireflow_process.png')
            print(f"  Saved FireFlow process grid ({n_total} steps): {args.workdir / 'progress_fireflow' / f'{img_name}_fireflow_process.png'}")
            del fireflow_progress_tensors, fireflow_progress_grid
        
        del fireflow_process_images
        torch.cuda.empty_cache()
        gc.collect()
        ...
        # ── 5. Evaluate & Save ───────────────────────────────────────
        # FireFlow results
        metrics_fireflow = compute_psnr_ssim(recon_img_fireflow_01, gt_img_01)
        ...
        save_image(recon_img_fireflow_01, args.workdir / 'recon_fireflow' / f'{img_name}.png')
        ...
        # Save comparisons
        save_comparison(
            y_vis_01, recon_img_fireflow_01, gt_img_01,
            args.workdir / 'recon_GT' / f'{img_name}_fireflow_comparison.png',
        )
        print(f"  Saved: {args.workdir / 'recon_GT' / f'{img_name}_fireflow_comparison.png'}")
        ...
        del recon_img_fireflow, recon_img_fireflow_01
        if args.use_posterior_sampling:
...
if __name__ == "__main__":
    ...
    args.workdir.joinpath('recon_fireflow').mkdir(parents=True, exist_ok=True)
    if args.use_posterior_sampling:

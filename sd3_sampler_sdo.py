...
class SD3Euler(StableDiffusion3Base):
    ...
    def euler_sample_wo_process(
        ...
        """
        Original full-backpropagation Euler sampling (used as fallback or reference).
        All steps retain the computational graph → high memory cost.
        """
        z = initial_z.clone().to(dtype=self.dtype)
        batch_size = z.shape[0]
        ts_buffer = torch.empty(batch_size, device=self.device, dtype=timesteps.dtype)
    ...
    def euler_sample_wo_process_SDO(
        self,
        initial_z: torch.Tensor,
        timesteps: torch.Tensor,
        sigmas: torch.Tensor,
        ...
        """
        Shortcut Diffusion Optimization (SDO) variant of Euler sampling.

        Motivation (from SDO paper, Sec. 4.2):
        ─────────────────────────────────────
        When optimizing the initial noise x_N (= initial_z here), plain
        backpropagation requires storing the full Jacobian product across
        all N denoising steps (Eq. 11a in the paper), which is expensive
        in both memory and time.

        SDO observes that, under Picard-iteration fixed-point dynamics,
        the gradient shortcut (one-step gradient, Eq. 15) is sufficient:

            ∇_{x_N} J(x_0)  ≈  (∂J/∂x_0) · (∂x_{N-1}/∂x_N)

        i.e., only the *first* denoising step (step_i == 0) needs to
        retain its computational graph.  All subsequent steps are run
        under torch.no_grad(), which:
          • eliminates storage of intermediate activations for steps 1…N-1,
          • avoids gradient explosion through the long trajectory, and
          • reduces computation time by ~90% vs. full backprop
            (consistent with Table 3 in the SDO paper).

        Practical implementation (mirrors Fig. 4 of the paper):
        ────────────────────────────────────────────────────────
        for i, t in enumerate(scheduler.timesteps):
            with torch.set_grad_enabled(i == 0):   # ← only step 0
                noise_pred = model(x_t, t)
            x_t = scheduler.step(noise_pred, t, x_t)

        Here `scheduler.step` (z = z + pred_v * dt) is always executed
        with gradients enabled so that ∂x_{N-1}/∂x_N propagates through
        the Euler update at step 0.

        Precision note (--use_amp):
        ───────────────────────────
        SDO's shortened backprop path greatly reduces gradient explosion,
        so mixed-precision (AMP) remains safe and is fully supported.
        The one SDO forward pass through the transformer (step 0) is
        wrapped by the caller's autocast context when --use_amp is set,
        which is fine because GradScaler handles the FP16→FP32 unscaling
        ...
        for step_i in range(n_steps):
            ...
            # ── SDO core: only step 0 keeps the computational graph ──
            # For step_i == 0  → grad enabled  (shortcut backprop path)
            # For step_i >= 1  → no_grad        (sg(·) in Eq. 18)
            with torch.set_grad_enabled(step_i == 0):
                pred_v = self.predict_vector(z, ts_buffer, prompt_emb, pooled_emb)
            ...
            # scheduler.step (Euler update) always runs with grad so that
            # ∂x_{N-1}/∂x_N is captured at step 0. For step_i >= 1 both
            # pred_v and z are already detached (no_grad above), so no
            # extra graph is built.
            sigma      = sigmas[step_i]
            sigma_next = sigmas[step_i + 1]
            z = z + pred_v * (sigma_next - sigma)

            # After step 0 the graph has been captured; detach z so that
            # subsequent Euler updates do not extend the graph further.
            if step_i == 0:
                # Keep z as a leaf-like tensor that still carries the
                # gradient path from initial_z through step 0.
                # We do NOT detach here — z still participates in the
                # graph via the step-0 computation.
                pass
